...
- **The whitelist is enforced at ingestion, not generation.** Only four domains (UBC, IRCC, BC, CRA) are ever crawled; a chunk without full provenance (`url`, `title`, `organisation`, `fetched_at`) is discarded before it can be used.
