...
const STATE_BADGE: Record<Step["state"], { label: string; className: string }> = {
  verified: { label: "Verified", className: "bg-emerald-50 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-300" },
  stale: { label: "Recheck due", className: "bg-amber-50 text-amber-800 dark:bg-amber-950/40 dark:text-amber-300" },
  no_source: { label: "Source pending", className: "bg-zinc-100 text-zinc-600 dark:bg-zinc-900 dark:text-zinc-400" },
  not_specific: { label: "General guidance", className: "bg-zinc-100 text-zinc-600 dark:bg-zinc-900 dark:text-zinc-400" },
};
...
function StepCard({ arrivalDate, step }: { arrivalDate: Date; step: Step }) {
  ...
  const hasContent = step.state !== "no_source";

  return (
    <li className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800">
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-1">
          <span className="text-xs font-medium text-zinc-500 dark:text-zinc-500">{formatDate(date)}</span>
      ...
      {hasContent ? (
        <>
          <p className="mt-2 text-sm text-zinc-700 dark:text-zinc-300">{step.why}</p>
          ...
          {step.sources.length > 0 && (
            <p className="mt-2 text-xs text-zinc-400 dark:text-zinc-600">
              Source: {step.sources[0].organisation} — {step.sources[0].title}
            </p>
          )}
        </>
      ) : (
